...
def main():
    ...
    L = ["# ORIN-SWEEP-1 — MORNING_SUMMARY (ledger F3) — root `orin-sweep-%s`" % a.stamp, "",
         ...
         "| planned | executed (cells present) | COMPLETED | FAILED | INVALID_THERMAL | delta-unverified | truncated (planned − executed) |", "|---|---|---|---|---|---|---|",
         "| %d | %d | %d | %d | %d | %d | %d |" % (acct["planned"], C["cells_total"], C["completed"], C["failed"], C["invalid_thermal"], C["delta_unverified"], acct["planned"] - C["cells_total"]), "",
    ...
    L += ["KAIST extension rule ({35, 25}) fires: **%s**." % A["kaist_extension_rule_fires"], "",
          ...
          "### F3 status: **%s**%s" % (A["F3"], (" (clause%s %s)" % ("s" if len(A["F3_clauses_true"]) > 1 else "", ", ".join(A["F3_clauses_true"]))) if A["F3_clauses_true"] else " — all three clauses false; the thesis bracket resolves to exact-equivalence-only; the operating-point characterization below stands on its own"), "",
