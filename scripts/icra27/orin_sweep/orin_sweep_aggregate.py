...
def main():
    ...
    result = {"schema": "schurvio.icra27.orin_sweep.aggregate.v1", "root": str(a.root), "idle_mean_mW_C5": idle, "idle_samples": idle_n,
              ...
              "counts": {"cells_total": len(rows), "completed": sum(r["status"] == "COMPLETED" for r in rows),
                         "failed": sum(r["status"] not in ("COMPLETED", "IN_PROGRESS") for r in rows), "invalid_thermal": sum(r["thermal"] == "INVALID_THERMAL" for r in rows),
                         "delta_unverified": sum((not r["delta_verified"]) and r["status"] != "IN_PROGRESS" for r in rows), "valid_for_stats": len(valid),
