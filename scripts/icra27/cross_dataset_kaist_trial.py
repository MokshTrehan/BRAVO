#!/usr/bin/python3.8
"""Append-only fresh-KAIST adapter for the CDSC-1 U0/S1 comparison.
...
def validate_canonical_campaign(protocol: Path, matrix: Path) -> Dict[str, Any]:
    if protocol != CANONICAL_PROTOCOL.resolve(strict=True):
        raise TrialError("protocol is not the canonical CDSC-1 path")
    if matrix != CANONICAL_MATRIX.resolve(strict=True):
        raise TrialError("matrix is not the canonical CDSC-1 path")
...
def launch_arguments(
    ...
    if bag_duration != -1.0:
        raise TrialError("CDSC-1 KAIST must run from the frozen start to bag end")
    paths = {
...
def kaist_pair_census(
    ...
    if bag_start != 0.0 or bag_duration != -1.0:
        raise TrialError("CDSC-1 KAIST census requires the complete adapted bag")
...
def _start_geometry_recorder(
    ...
    recorder_name = "icra27_cdsc1_kaist_geometry_recorder"
...
def _matrix_rotation_declared_binding(matrix_path: Path) -> Dict[str, Any]:
    ...
    matrix_path = common._regular_file(matrix_path, "CDSC-1 matrix")
    if matrix_path != CANONICAL_MATRIX.resolve(strict=True):
        raise TrialError("post-pair matrix is not the canonical CDSC-1 path")
    try:
        matrix = yaml.safe_load(matrix_path.read_text(encoding="utf-8", errors="strict"))
    except yaml.YAMLError as exc:
        raise TrialError("CDSC-1 matrix is invalid YAML") from exc
